---
description: Core design principles governing all code in Otto
---
...
- All commands route through `Ottogen::Ottogen` class methods; the CLI layer (`Ottogen::Otto`) only dispatches.
- `error_if_not_otto_project` is the project-root guard — every command except `init` calls it. An otto project is identified by the sentinel `.otto` file at the working directory root.
- Build output goes to `_build/`; pages live under `pages/`; static files under `assets/`.
- User feedback uses emoji-prefixed `puts` lines (✨ 🔨 📝 🧽 👀 🤖 ✅ ❌). Match the style when adding new commands.
